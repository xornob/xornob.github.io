...
function reset () {
	...
	gameh.style.left ='0px';
	gameh.style.right='0px';
}
function swapback () {
...
function movetext () {
	gameh.style.left=(gameh.getBoundingClientRect().left-Math.floor(speed))+'px';
	speed+=0.0005;
	if (gamen.getBoundingClientRect().right>-5) {
		gamen.innerHTML+=' '+words[Math.floor(Math.random()*959)];
	};
	if (gameh.getBoundingClientRect().left<-300) {
		gameh.style.left=(gameh.getBoundingClientRect().left+180)+'px';
...
document.onkeydown=function(e) {
	...
	if(!playing) {
		playing=1;
		loop = window.setInterval(movetext,20);
		gamed.innerHTML='Press any key to ';
		if (key==66) {
